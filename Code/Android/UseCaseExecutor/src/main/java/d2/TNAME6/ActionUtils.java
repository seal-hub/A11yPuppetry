...
public class ActionUtils {
    ...
    static class DefaultGestureCallback extends AccessibilityService.GestureResultCallback {
        ...
        public void onCompleted(GestureDescription gestureDescription) {
            Log.i(TOOL_NAMEService.TAG, "Complete Gesture " + gestureDescription.toString());
        ...
        public void onCancelled(GestureDescription gestureDescription) {
            Log.i(TOOL_NAMEService.TAG, "Cancel Gesture " + gestureDescription.toString());
            if (actionCallback != null)
    ...
    public static ActualWidgetInfo findActualWidget(ConceivedWidgetInfo targetWidget){
        ...
        if(similarNodes.size() != 1){
            if(similarNodes.size() == 0) {
                Log.e(TOOL_NAMEService.TAG, "The target widget could not be found in current screen.");
                // TODO: For debugging
                Log.d(TOOL_NAMEService.TAG, "The target XPATH: \n\t" + targetWidget.getXpath());
                ...
                for(AccessibilityNodeInfo nodeInfo : allNodes){
                    ...
                    if (actualWidgetInfo != null)
                        Log.d(TOOL_NAMEService.TAG, "\t" + actualWidgetInfo.getXpath());
                }
            }
            else{
                Log.d(TOOL_NAMEService.TAG, "There are more than one candidates for the target.");
                for(AccessibilityNodeInfo node : similarNodes){
                    Log.d(TOOL_NAMEService.TAG, " Node: " + node);
                }
            }
            return null;
        }
    ...
    public static boolean isFocusedNodeTarget(List<AccessibilityNodeInfo> similarNodes) {
        ...
        if(!isSimilar) {
            if (focusedNode != null && focusedNode.getChildCount() == 0){
                Log.i(TOOL_NAMEService.TAG, "-- TalkBack is focused on a leaf node" + focusedNode);
                ...
                Log.i(TOOL_NAMEService.TAG, "-- TalkBack is focused on a parent node" + focusedNode);
            ...
            Log.i(TOOL_NAMEService.TAG, "-- FIRST REACHABLE NODE IS " + firstReachableNode);
            isSimilar = firstReachableNode != null && firstReachableNode.equals(TOOL_NAMEService.getInstance().getAccessibilityFocusedNode());
        }
        return isSimilar;
    }
    ...
    public static Pair<Integer, Integer> getClickableCoordinate(AccessibilityNodeInfo node, boolean excludeDescendantsBounds){
        ...
        if(excludeDescendantsBounds)
        {
            ...
            Log.i(TOOL_NAMEService.TAG, " -------> " + nodeBox + " " + firstDescendantLeft + " " + lastDescendantRight + " " + firstDescendantTop + " " + lastDescendantBottom);
    ...
    public static boolean performBack(){
        return TOOL_NAMEService.getInstance().performGlobalAction(AccessibilityService.GLOBAL_ACTION_BACK);
    }
    ...
    public static boolean performTap(int x, int y, int startTime, int duration, AccessibilityService.GestureResultCallback callback){
        ...
        Log.i(TOOL_NAMEService.TAG, "Execute Gesture " + gestureDescription.toString());
        return TOOL_NAMEService.getInstance().dispatchGesture(gestureDescription, callback, null);
    }

    public static boolean performThreeFingerTap(AccessibilityService.GestureResultCallback callback){
        GestureDescription.Builder gestureBuilder = new GestureDescription.Builder();
        int BASE = 50;
        WindowManager wm = (WindowManager) TOOL_NAMEService.getInstance().getApplicationContext().getSystemService(Context.WINDOW_SERVICE);
        ...
        GestureDescription gestureDescription = gestureBuilder.build();
        Log.i(TOOL_NAMEService.TAG, "Execute ThreeFingerTap Gesture " + gestureDescription.toString());
        return TOOL_NAMEService.getInstance().dispatchGesture(gestureDescription, callback, null);
    }

    public static boolean performType(AccessibilityNodeInfo node, String message){
        Log.i(TOOL_NAMEService.TAG, "performType");
    ...
    public static boolean performDoubleTap(final AccessibilityService.GestureResultCallback callback){
        Log.i(TOOL_NAMEService.TAG, "performDoubleTap");
        try {
    ...
    public static boolean performDoubleTap(final int x, final int y, final int startTime, final int duration, final AccessibilityService.GestureResultCallback callback){
        AccessibilityService.GestureResultCallback newClickCallBack = new AccessibilityService.GestureResultCallback() {
            @Override
            public void onCompleted(GestureDescription gestureDescription) {
                Log.i(TOOL_NAMEService.TAG, "Complete Gesture " + gestureDescription.getStrokeCount());
                super.onCompleted(gestureDescription);
                try {
            ...
            @Override
            public void onCancelled(GestureDescription gestureDescription) {
                Log.i(TOOL_NAMEService.TAG, "Cancel Gesture");
                super.onCancelled(gestureDescription);
                if(callback != null)
    ...
    private static void executeSwipeGesture(String direction, String secondDirection, AccessibilityService.GestureResultCallback callback){
        ...
        switch (direction) {
            ...
            case "down":
                ...
                break;
            default:
                Log.e(TOOL_NAMEService.TAG, "Incorrect direction " + direction);
                return;
        }
        ...
        if (!secondDirection.isEmpty())
        {
            ...
            {
                Log.i(TOOL_NAMEService.TAG, "Add left direction to " + Integer.toString(100+dx1) + " " +Integer.toString(y2-dy1));
                swipePath.lineTo(100+dx1, y2-dy1);
            }
            if (direction.equals("left") && secondDirection.equals("down"))
            {
                Log.i(TOOL_NAMEService.TAG, "Add down direction to " + Integer.toString(100+dx1) + " " +Integer.toString(y2-dy1));
        ...
        GestureDescription gestureDescription = gestureBuilder.build();
        Log.i(TOOL_NAMEService.TAG, "Execute Gesture " + gestureDescription.toString());
        TOOL_NAMEService.getInstance().dispatchGesture(gestureDescription, callback, null);
    }
    ...
    public static boolean swipeToDirection(String direction, String secondDirection, ActionCallback doneCallback){
        if(isActionPending()){
            Log.i(TOOL_NAMEService.TAG, String.format("Do nothing since another action is pending! (Size:%d)", pendingActions.size()));
            return false;
        }
        ...
        AccessibilityService.GestureResultCallback callback = new AccessibilityService.GestureResultCallback() {
            @Override
            public void onCompleted(GestureDescription gestureDescription) {
                Log.i(TOOL_NAMEService.TAG, "Gesture execution is completed!");
                new Handler().postDelayed(() -> {
                    pendingActions.remove(thisActionId);
                    if(doneCallback != null)
                        doneCallback.onCompleted(TOOL_NAMEService.getInstance().getAccessibilityFocusedNode());
            ...
            @Override
            public void onCancelled(GestureDescription gestureDescription) {
                Log.i(TOOL_NAMEService.TAG, "Gesture is cancelled!");
                pendingActions.remove(thisActionId);
                if(doneCallback != null)
    ...
    public static boolean a11yFocusOnNode(AccessibilityNodeInfo node){
        AccessibilityNodeInfo currentFocusedNode = TOOL_NAMEService.getInstance().getAccessibilityFocusedNode();
        ...
        if (focusableWidget == null) {
            Log.e(TOOL_NAMEService.TAG, "The requested focusing  widget is null!");
            return false;
        }
        Log.i(TOOL_NAMEService.TAG, "Focusing on widget: " + focusableWidget.completeToString(true));
    ...
    public static boolean focusOnNode(AccessibilityNodeInfo node){
        AccessibilityNodeInfo currentFocusedNode = TOOL_NAMEService.getInstance().getFocusedNode();
        if (currentFocusedNode != null)
