...
TOOL_NAME_INTENT = "PREFIX.TOOL_NAME.COMMAND"
IS_LIVE_FILE_PATTERN = "is_live_{0}.txt"


def _encode_TOOL_NAME_message(message: str) -> str:
    message = message \
        .replace('"', "__^__") \
        .replace(" ", "__^^__") \
        .replace(",", "__^^^__") \
...
async def send_command_to_TOOL_NAME(command: str, extra: str = "NONE", device_name: str = DEVICE_NAME) -> bool:
    logger.debug(f"Sending command {command} with extra {extra} to TOOL_NAME!")
    extra = _encode_TOOL_NAME_message(extra)
    bash_cmd = f'adb -s {device_name} shell am broadcast -a {TOOL_NAME_INTENT} --es command "{command}" --es extra "{extra}"'
...
async def send_commands_sequence_to_TOOL_NAME(command_sequence: List[Union[str, Tuple[str, str]]],
    ...
    return await send_command_to_TOOL_NAME('sequence', json.dumps(command_extra), device_name=device_name)


async def is_TOOL_NAME_live(device_name: str = DEVICE_NAME) -> bool:
    random_message = ''.join(random.choice(string.ascii_lowercase) for i in range(20))
    if not await send_command_to_TOOL_NAME('is_live', random_message, device_name=device_name):
        return False
