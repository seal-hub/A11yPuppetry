...
from consts import TNAME2_TAG, TNAME2_EVENTS_TAG, CAPTURE_STATE_DELAY
from json_util import JSONSerializable
from TOOL_NAME_executor_utils import TOOL_NAME_capture_layout
...
class WebHelper:
    ...
    def get_events_info(self, mode, index) -> dict:
        event_log_path = self.address_book.get_log_path(mode, index, extension=TNAME2_EVENTS_TAG)
...
class AddressBook:
    ...
    def get_log_path(self, mode: str, index: int, extension: str = None, should_exists: bool = False):
        file_name = f"{index}_{extension}.log" if (
                extension is not None and extension != TNAME2_TAG) else f"{index}.log"
        return self._get_path(mode, file_name, should_exists)
        ...
        if self.action_path.exists():
            with open(self.action_path) as f:
                for action in f.readlines():
                    ...
                    with open(self.get_log_path('tb', action['index'], extension=TNAME2_EVENTS_TAG)) as f2:
            ...
            with open(self.s_action_path) as f:
                for action in f.readlines():
                    ...
                    if action['tb_action_result'] is not None:
                        with open(self.get_log_path('s_tb', action['index'], extension=TNAME2_EVENTS_TAG)) as f2:
                            if "TYPE_VIEW_CLICKED" in f2.read():
                                tb_actions_xpaths[action['element']['xpath']] = action
                    with open(self.get_log_path('s_areg', action['index'], extension=TNAME2_EVENTS_TAG)) as f2:
                        if "TYPE_VIEW_CLICKED" in f2.read():
...
async def capture_layout(device) -> Tuple[str, dict]:
    ...
    return await padb_logger.execute_async_with_log(TOOL_NAME_capture_layout(device_name=device.serial))
...
class ResultWriter:
    ...
    def visit_element(self, visited_element: dict, state: str, node: Union[Node, None]) -> None:
        ...
        :param visited_element: The element that is visited by TOOL_NAME
