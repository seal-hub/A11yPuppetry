...
from TOOL_NAME_utils import send_commands_sequence_to_TOOL_NAME, send_command_to_TOOL_NAME
...
class Controller(ABC):
    ...
    async def execute(self, command: Command, first_setup: bool = False, **kwargs) -> CommandResponse:
        ...
        for i in range(ACTION_EXECUTION_RETRY_COUNT):
            ...
            await send_command_to_TOOL_NAME(command="controller_execute",
            ...
            if result is None or len(result.strip()) == 0:
                ...
                await send_command_to_TOOL_NAME("controller_interrupt", device_name=self.device.serial)
            else:
                try:
...
class TouchController(Controller):
    ...
    async def setup(self):
        await A11yServiceManager.setup_TOOL_NAME_a11y_services(tb=False, device_name=self.device.serial)
        await send_commands_sequence_to_TOOL_NAME([("controller_set", self.mode())], device_name=self.device.serial)
...
class TalkBackAPIController(Controller):
    ...
    async def setup(self):
        await A11yServiceManager.setup_TOOL_NAME_a11y_services(tb=True, device_name=self.device.serial)
        await send_commands_sequence_to_TOOL_NAME([("controller_set", self.mode())], device_name=self.device.serial)
...
class TalkBackSearchController(Controller):
    ...
    async def execute(self, command: Command, first_setup: bool = False,
        ...
        # Open Search Screen
        await send_command_to_TOOL_NAME(command="tb_search", device_name=self.device.serial)
