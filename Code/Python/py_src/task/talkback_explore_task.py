...
from consts import TNAME2_TAG, TNAME2_EVENTS_TAG, EXPLORE_VISIT_LIMIT, MAX_DIRECTIONAL_NAVIGATION
...
def is_window_changed(log_message_map):
    try:
        ...
        for line in log_message_map[TNAME2_EVENTS_TAG].split("\n"):
...
class TalkBackExploreTask(SnapshotTask):
    ...
    async def is_target_node_found(self, padb_logger, focused_node: Node, controller: Controller, tags) -> Tuple[bool, str, str]:
        ...
        android_logs = f"--------- First Focused Node -----------\n" \
                        f"{log_message_map[TNAME2_TAG]}\n" \
                        f"----------------------------------------\n"
        android_event_logs = f"{log_message_map[TNAME2_EVENTS_TAG]}\n"
    ...
    async def execute(self) -> bool:
        ...
        android_logs = ""
        android_event_logs = ""
        tags = [TNAME2_TAG, TNAME2_EVENTS_TAG]
