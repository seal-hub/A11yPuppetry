...
from consts import LAYOUT_TIMEOUT_TIME, \
    TNAME2_TAG, TB_TREELIST_TAG, DEVICE_NAME
from TOOL_NAME_utils import send_command_to_TOOL_NAME
...
async def TOOL_NAME_capture_layout(device_name: str = DEVICE_NAME) -> str:
    layout = None
    for i in range(3):
        logger.debug(f"Capturing layout, Try: {i}")
        await send_command_to_TOOL_NAME("capture_layout", device_name=device_name)
    ...
    try:
        ...
        logger.error(f"Exception during capturing layout in TOOL_NAME: {e}")
        layout = f"PROBLEM_WITH_XML {random.random()}"
...
async def report_atf_issues(device_name: str = DEVICE_NAME) -> List:
    ...
    for i in range(3):
        ...
        await A11yServiceManager.enable('TOOL_NAME', device_name=device_name)
        await send_command_to_TOOL_NAME("report_a11y_issues", device_name=device_name)
...
async def report_tb_focusables(device_name: str = DEVICE_NAME) -> List[Node]:
    ...
    for i in range(3):
        ...
        await A11yServiceManager.enable('TOOL_NAME', device_name=device_name)
        await send_command_to_TOOL_NAME("report_tb_nodes", device_name=device_name)
...
async def talkback_tree_nodes(padb_logger: ParallelADBLogger,
    ...
    """
    First enables TalkBack and TOOL_NAME, then send "tb_a11y_tree" command to TOOL_NAME, and observes the logs
    from both TOOL_NAME and TalkBack (which should contains the Virtual View Hierarchy). Finally, returns a
    ...
    :param device_name:
    :return: A tuple of WindowInfo and TOOL_NAME logs. WindowInfo is a map from window id to a map of
    ...
    async def send_a11y_tree_command():
        await send_command_to_TOOL_NAME("tb_a11y_tree", device_name=device_name)
        await asyncio.sleep(2)

    # TODO: At the end of this function, the a11y serviecs should be returned to the initial state
    await A11yServiceManager.setup_TOOL_NAME_a11y_services(tb=True, device_name=device_name)
    logs, next_command_str = await padb_logger.execute_async_with_log(
        send_a11y_tree_command(),
        tags=[TNAME2_TAG, TB_TREELIST_TAG])
    flag = False
    ...
    return windows_info, logs[TNAME2_TAG]