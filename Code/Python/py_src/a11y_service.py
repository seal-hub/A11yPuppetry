...
# TODO: Need to decompose TOOL_NAME_utils into TOOL_NAME_comms and TOOL_NAME_navigations
from TOOL_NAME_utils import is_TOOL_NAME_live
...
class A11yServiceManager:
    services = {"tb": "com.google.android.marvin.talkback/com.google.android.marvin.talkback.TalkBackService",
                "TOOL_NAME": "PREFIX.TOOL_NAME/PREFIX.TOOL_NAME.app.MyTOOL_NAMEService"}
    ...
    @staticmethod
    async def setup_TOOL_NAME_a11y_services(tb=False, device_name: str = DEVICE_NAME) -> None:
        requested_services = ["TOOL_NAME"]
        if tb:
        ...
        live_TOOL_NAME = False
        for i in range(10):
            if await is_TOOL_NAME_live(device_name=device_name):
                live_TOOL_NAME = True
                break
            else:
                logger.info(f"Waiting for TOOL_NAME to be alive...")
        if not live_TOOL_NAME:
            # TODO: too harsh, it's better to return live_TOOL_NAME and let the outer method decides
            raise "TOOL_NAME is not alive"
